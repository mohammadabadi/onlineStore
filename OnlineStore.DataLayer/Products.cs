...
namespace OnlineStore.DataLayer
{
    ...
    public static class Products
    {
        ...
        public static List<OnlineStore.Models.Public.ProductGroup> GetProductGroups()
        {
            using (var db = new OnlineStoreDbContext())
            {
                return (from g in db.ProductGroups
                        select new OnlineStore.Models.Public.ProductGroup()
                        {
                            GroupId = g.GroupID,
                            Id = g.ID,
                            ProductId = g.ProductID,
                            GroupText = g.Group == null ? "" : g.Group.Text,
                            GroupTitle = g.Group == null ? "" : g.Group.Title
                        }).ToList();
            }
        }
        ...
        public static int Count(List<int> groupIDs, List<string> attributes = null, List<int> producers = null)
        {
            using (var db = OnlineStoreDbContext.Entity)
            {
                ...
                if (attributes != null && attributes.Count > 0)
                {
                    ...
                    {
                        query = query.Where(item => db.AttributeValues
                        .Any(av => av.ProductID == item.ID && av.AttributeID == attr.AttrID && av.AttributeOptionID == attr.OpID));
                    }
                }
