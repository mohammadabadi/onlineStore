...
namespace OnlineStore.Website.Controllers
{
    public class ProductsController : PublicController
    {
            ...
            string attributes = "")
        {
            ...
            {
                ViewBag.Description = "خرید آنلاین " + groupsProducers.Take(10).Select(item => item.Title + " (" + item.TitleEn + ")").Aggregate((a, b) => b + ", " + a) + " درسامانه فروش آنلاین الیس سانیا" + (pageIndex > 0 ? " - صفحه " + (pageIndex + 1) : String.Empty);
        ...
        public ActionResult Details(int id)
        {
            ...
            ViewBag.Description =
                ...
                "خرید آنلاین و بررسی تخصصی " + productDetail.Title + " در فروشگاه اینترنتی الیاس سانیا"
                );
        ...
        public JsonResult AddRequest(int productID, string email, string mobile, string description)
        {
            ...
            string productLink = "https://lasenor.ir" + UrlProvider.GetProductUrl(productID, group.UrlPerfix, product.UrlPerfix);
